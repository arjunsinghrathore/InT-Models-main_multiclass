...
class hConvGRUCell(nn.Module):
    ...
    def forward(self, input_, inhibition, excitation,  activ=F.softplus, testmode=False):  # Worked with tanh and softplus
        ...
        if not self.no_inh:
            ...
            # inh_gate = self.att_nl(self.i_w_gate(gated_input) + self.i_u_gate(gated_inhibition))
        ...
        # Multi Class Classification
        # exc_gate = self.att_nl(self.e_w_gate(gated_inhibition) + self.e_u_gate(gated_excitation))
...
class FFhGRU(nn.Module):
    ...
    def __init__(self, dimensions, timesteps=8, kernel_size=15, jacobian_penalty=False, grad_method='bptt', no_inh=False, lesion_alpha=False, lesion_mu=False, lesion_gamma=False, lesion_kappa=False, nl=F.softplus, classes = 3):
        '''
        '''
        ...
        self.readout_conv = nn.Conv2d(dimensions, self.classes, 1)
        self.target_conv = nn.Conv2d(self.classes+1, self.classes, 5, padding=5 // 2)
        ...
        self.readout_dense = nn.Linear(self.classes, self.classes)
