...
def multi_acc(y_test, y_pred, n_classes, only_acc = False):
    ...
    # Precision
    precision = Precision(average='samples', num_classes=n_classes)
    precision_val = precision(y_pred_tags, y_test)

    # Recall
    recall = Recall(average='samples', num_classes=n_classes)
    recall_val = recall(y_pred_tags, y_test)

    # F1 Score
    f1 = F1(average='samples', num_classes=n_classes)
    f1_val = f1(y_pred_tags, y_test)
    
    if only_acc:
        return acc
    else:
        acc = acc * 100
        return acc, precision_val, recall_val, f1_val
