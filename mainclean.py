...
def validate(val_loader, model, criterion, device, logiters=None):
    ...
    with torch.no_grad():
        for i, (imgs, target) in enumerate(val_loader):
            ...
            # _, output_ls = torch.max(y_pred_softmax, dim = 1)    
            # output_ls = output_ls.int()
            # target = target.int()
            
            # Binary Classification
            # loss = criterion(output, target.float().reshape(-1, 1))
            # Multi Class Classification   
            loss = criterion(output, target.long().reshape(-1))
            # MSE LOSS
            # loss = criterion(output_ls, target.reshape(-1))

            # Binary Classification
            # prec1, preci, rec, f1s = acc_scores(target[:], output.data[:])
            # Multi Class Classification   
            prec1, preci, rec, f1s = multi_acc(target[:], output.data[:])
...
if __name__ == '__main__':
    ...
    results_folder = os.path.join('results_' + str(args.classes-1) + '_targets_' + str(args.height) + '_2', stem, '{0}'.format(args.name))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for idx, (imgs, target) in enumerate(train_loader):
            ...
            # prec1, preci, rec, f1s = acc_scores(target[:], output.data[:])
            # Multi Class Classification   
            prec1, preci, rec, f1s = multi_acc(target[:], output.data[:], n_classes = args.classes)
